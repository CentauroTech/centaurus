...
export const MemoizedTaskRow = memo(function MemoizedTaskRow({
  ...
  const renderCell = useCallback((column: ColumnConfig) => {
    ...
    switch (column.type) {
      ...
      case 'number':
        return <NumberCell value={value as number} onChange={val => handleUpdate(column.field, val)} displayFormat={column.field === 'cantidadEpisodios' ? 'episodes' : undefined} episodeIndex={column.field === 'cantidadEpisodios' ? task.episodeIndex : undefined} />;
      case 'date':
