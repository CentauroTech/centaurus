...
export function TaskDetailsPanel({
  ...
  } = useComments(task.id, isOpen);
  const {
    data: activityLogs,
    isLoading: activityLoading
  } = useActivityLog(task.id, isOpen);
  const {
    data: teamMembers
  } = useTeamMembers();
  const {
  ...
  const filteredMentionUsers = teamMembers?.filter(member => mentionSearch === '' || member.name.toLowerCase().includes(mentionSearch.toLowerCase())).slice(0, 6) || [];
  ...
  const handleSendComment = async () => {
    ...
    if (teamMembers && mentionedNames.length > 0) {
      for (const name of mentionedNames) {
        const member = teamMembers.find(m => m.name.toLowerCase() === name.toLowerCase() || m.name.toLowerCase().replace(/\s+/g, '') === name.toLowerCase().replace(/\s+/g, ''));
        if (member) {
      ...
      <SheetContent className="w-[480px] sm:max-w-[480px] p-0 flex flex-col">
        ...
        <Tabs defaultValue="updates" className="flex-1 flex flex-col overflow-hidden">
          <TabsList className="w-full justify-start rounded-none border-b border-border bg-transparent px-4 h-12">
            <TabsTrigger value="updates" className="data-[state=active]:border-b-2 data-[state=active]:border-primary data-[state=active]:bg-transparent rounded-none">
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
                <div className="flex border-b border-border bg-muted/20">
                  <button onClick={() => setActiveUpdateTab('team')} className={cn("flex-1 px-4 py-2 text-sm font-medium transition-colors", activeUpdateTab === 'team' ? "border-b-2 border-primary text-foreground" : "text-muted-foreground hover:text-foreground")}>
                    <Users className="w-4 h-4 inline mr-2" />
                    Team Updates
                  </button>
                  <button onClick={() => setActiveUpdateTab('guest')} className={cn("flex-1 px-4 py-2 text-sm font-medium transition-colors", activeUpdateTab === 'guest' ? "border-b-2 border-primary text-foreground" : "text-muted-foreground hover:text-foreground")}>
                    <UserIcon className="w-4 h-4 inline mr-2" />
                    Guest Communication
                  </button>
                </div>
                
                {activeUpdateTab === 'team' ? <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
                    <CommentSection title="Internal Team Updates" icon="team" comments={teamComments} isLoading={commentsLoading} onSendComment={handleSendToSection} teamMembers={teamMembers || []} isGuestVisible={false} emptyMessage="No team updates yet" />
                  </div> : <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
                    <CommentSection title="Guest Communication" icon="guest" comments={guestComments} isLoading={commentsLoading} onSendComment={handleSendToSection} teamMembers={teamMembers || []} isGuestVisible={true} emptyMessage="No guest communication yet" />
                  </div>}
              </div>}
          </TabsContent>
          ...
          <TabsContent value="files" className="flex-1 m-0 overflow-hidden flex flex-col">
            <div className="p-4 border-b border-border flex justify-between items-center">
              ...
              {!isGuest && <FileUploadButton onUpload={async (file, category) => {
            ...
            }} isUploading={uploadFile.isPending} currentPhase={currentPhase} />}
            </div>
            <ScrollArea className="flex-1 p-4">
              {filesLoading ? <div className="text-center py-12 text-muted-foreground">
                  <p className="text-sm">Loading files...</p>
                </div> : taskFiles && taskFiles.length > 0 ? <div className="space-y-6">
                  {FILE_CATEGORIES.map(cat => <FileCategorySection key={cat.value} title={cat.label} files={filesByCategory[cat.value] || []} isGuest={isGuest} onToggleAccess={(fileId, isAccessible) => toggleFileAccess.mutate({
                fileId,
                isGuestAccessible: isAccessible
          ...
          <TabsContent value="activity" className="flex-1 m-0 overflow-hidden">
            <ScrollArea className="h-full p-4">
              {activityLoading ? <div className="text-center py-12 text-muted-foreground">
                  <p className="text-sm">Loading activity...</p>
                </div> : activityLogs && activityLogs.length > 0 ? <div className="space-y-4">
                  {activityLogs.map(item => <DatabaseActivityItem key={item.id} item={item} />)}
                </div> : <div className="text-center py-12 text-muted-foreground">
...
function renderCommentWithMentions(content: string): React.ReactNode {
  ...
  return parts.map((part, index) => {
    if (part.startsWith('@')) {
      return <span key={index} className="text-primary font-medium bg-primary/10 px-1 rounded">
          {part}
        </span>;
    }
    return part;
...
function CommentItem({
  ...
  return <div className="flex gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        {comment.attachments && comment.attachments.length > 0 && <div className="mt-2 flex flex-wrap gap-2">
            {comment.attachments.map(file => <div key={file.id} className="flex items-center gap-2 px-2 py-1 rounded bg-muted text-xs">
                {file.type === 'image' ? <Image className="w-3 h-3" /> : <File className="w-3 h-3" />}
                <span className="truncate max-w-[120px]">{file.name}</span>
              </div>)}
          </div>}
        {comment.reactions && comment.reactions.length > 0 && <div className="mt-2 flex gap-1">
            {comment.reactions.map((reaction, idx) => <span key={idx} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-muted text-xs">
...
function FileItem({
  ...
  return <div className="flex flex-col items-center p-4 rounded-lg border border-border hover:bg-accent/50 transition-smooth cursor-pointer">
      {getFileIcon()}
      <span className="mt-2 text-xs font-medium text-center truncate w-full">{file.name}</span>
      <span className="text-xs text-muted-foreground">{formatFileSize(file.size)}</span>
    </div>;
}
function DatabaseActivityItem({
  item
}: {
