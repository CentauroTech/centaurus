...
export function ProjectManagerCell({
  ...
  const users: User[] = projectManagers.map(m => ({
    id: m.id,
    name: m.name,
    initials: m.initials,
    color: m.color
  ...
  return <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && <div className="absolute z-[9999] mt-1 left-0 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 rounded-lg shadow-dropdown border border-border py-1 min-w-[180px] animate-fade-in">
          {users.length === 0 ? <div className="px-3 py-2 text-sm text-slate-500 dark:text-slate-400">
            ...
            </div> : users.map(user => <button key={user.id} onClick={() => {
        onOwnerChange(user);
        setIsOpen(false);
      }} className={cn("w-full px-3 py-2 text-left flex items-center gap-2 text-slate-900 dark:text-slate-100 hover:bg-slate-100 dark:hover:bg-slate-800 transition-smooth", owner?.id === user.id && "bg-slate-100 dark:bg-slate-800")}>
                <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium text-white" style={{
