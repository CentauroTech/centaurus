...
export function useCreateInvoice() {
  ...
  return useMutation({
    mutationFn: async (data: CreateInvoiceData & { asDraft?: boolean }) => {
      if (!currentMember?.id) throw new Error('Not authenticated');

      const isDraft = data.asDraft !== false; // default to draft
      ...
      if (!isDraft) {
        ...
        for (const bm of billingMembers || []) {
          await supabase.rpc('create_notification', {
            p_user_id: bm.team_member_id,
            p_type: 'invoice_submitted',
            p_title: `Invoice ${invoiceData.invoice_number} Submitted`,
            p_message: `${currentMember.name || 'A team member'} submitted an invoice for approval. invoice_id::${invoice.id}`,
            p_task_id: null,
