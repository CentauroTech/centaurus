-- Create a function to handle phase progression that bypasses RLS for board lookups
-- This is needed because guests can't read boards directly but need to trigger phase changes

CREATE OR REPLACE FUNCTION public.move_task_to_next_phase(
  p_task_id uuid,
  p_user_id uuid
)
RETURNS jsonb
