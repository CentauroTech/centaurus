...
BEGIN
  ...
  FROM task_groups tg WHERE tg.id = v_current_group_id;

  SELECT b.name, b.workspace_id INTO v_current_board_name, v_workspace_id
  FROM boards b WHERE b.id = v_current_board_id;
  ...
  v_next_phase := CASE LOWER(REGEXP_REPLACE(v_current_phase, '[^a-zA-Z0-9]', '', 'g'))
    ...
    WHEN 'adapting' THEN 'Breakdown'
    WHEN 'breakdown' THEN 
      CASE WHEN v_prueba_de_voz = 'Yes' THEN 'Casting' ELSE 'Recording' END
    WHEN 'casting' THEN 'Scheduling'
    WHEN 'voicetests' THEN 'Scheduling'
    WHEN 'scheduling' THEN 'Recording'
    WHEN 'recording' THEN 'QC 1'
    WHEN 'premix' THEN 'QC 1'
    WHEN 'qc1' THEN 'Retakes'
    WHEN 'qcpremix' THEN 'Retakes'
    WHEN 'retakes' THEN 'Mix'
  ...
  FROM task_groups tg WHERE tg.board_id = v_next_board.id
  ...
  UPDATE tasks SET 
    group_id = v_target_group_id, status = 'default', fase = v_next_phase_name,
    last_updated = v_now, date_assigned = v_current_date,
    date_delivered = NULL, guest_due_date = NULL
  WHERE id = p_task_id;
  ...
  IF v_normalized_next IN ('translation', 'adapting', 'adaptacion') THEN
    UPDATE tasks SET asignacion = 'On Hold' WHERE id = p_task_id;
  END IF;

  -- === AUTO-ASSIGN for Adapting: if adaptador_id is already set, auto-assign ===
  IF v_normalized_next IN ('adapting', 'adaptacion') THEN
    SELECT adaptador_id INTO v_role_member_id FROM tasks WHERE id = p_task_id;
    ...
    IF v_role_member_id IS NOT NULL THEN
      ...
      IF v_is_guest THEN
        ...
        UPDATE tasks 
        SET is_private = true, guest_due_date = v_guest_due, asignacion = 'Asignado'
        WHERE id = p_task_id;
        ...
        INSERT INTO activity_log (task_id, type, field, old_value, new_value, user_id)
        VALUES (p_task_id, 'field_change', 'asignacion', 'On Hold', 'Asignado', p_user_id);

        -- Skip the generic auto-privacy block below since we already handled it
  ...
  v_role_column := CASE v_normalized_next
    ...
    WHEN 'qc1' THEN 'qc_1_id'
    WHEN 'qcpremix' THEN 'qc_1_id'
    WHEN 'retakes' THEN 'qc_retakes_id'
